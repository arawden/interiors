...
my %OPTIONS = (
	format => '/me is listening to: {%a - %t on %b (%y)} (✿◠‿◠)', # see `man ncmpcpp` for song format
	shades => '/me is listening to: {%a - %t on %b (%y)} ヾ(⌐■_■)ノ',
);
my $TIMEOUT = 20 * 1000;
my $COMMANDS = "play|pause|toggle|stop|next|prev|shades";
...
sub command_cb
{
	...
	if ($args =~ /^((play)|(pause)|(toggle)|(stop)|(next)|(prev))$/i) {
		...
		$cmd = "--now-playing '$OPTIONS{shades}'";
	}
	elsif ($args =~ /^\s*$/) {
